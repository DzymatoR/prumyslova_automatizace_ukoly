...
app.layout = html.Div(
    [
        ...
        html.Div(
            id="city-title",
            style={"fontSize": "20px", "fontWeight": "600", "margin": "10px 0"},
        ),
...
def update_timeseries(clickData):
    ...
    if df24.empty:
        fig = go.Figure()
        fig.update_layout(
            title=f"Žádná hodinová data pro {city}", margin=dict(l=0, r=0, t=40, b=0)
        )
    ...
    fig = px.line(
        ...
        x="time",
        y="temperature_2m",
        title=f"Teplota za posledních 24 h – {city} ({region})",
        markers=True,
    )
    fig.update_layout(
        xaxis_title="Čas", yaxis_title="Teplota [°C]", margin=dict(l=0, r=10, t=40, b=0)
    )
